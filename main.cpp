...
Vec3f trace_ray(const Ray& ray, const Scene& scene, int depth) {
    ...
    if (scene.intersect(ray, hit)) {
        ...
        Vec3f albedo = hit.material ? material_albedo(*hit.material) : Vec3f(1.0f, 1.0f, 1.0f);
...
Image render_image(const RenderConfig& config, const Scene& scene, const Camera& camera) {
    ...
    for (int j = 0; j < config.image_height; ++j) {
        ...
        for (int i = 0; i < config.image_width; ++i) {
            ...
            image.pixels[j * config.image_width + i] = color;
        }
    }
...
int main(int argc, char** argv) {
    ...
    if (image.save_with_tonemapping(output_file)) {
